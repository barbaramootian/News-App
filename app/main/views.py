...
def index():
    ...
    bbc_news = get_articles_news('bbc-news')
    title = 'Welcome to Global News'
    return render_template('index.html', title=title, all_sources=all_sources,bbc_news=bbc_news)
...
def articles(name):
    ...
    """
    bbc_news = get_articles_news('bbc_news')
    top_headlines = get_articles_news(name)
    return render_template('news.html', top_headlines=top_headlines,bbc_news=bbc_news)
